...
public class WorldRenderer
{
    ...
    private VertexFormatElement matchingImportantField;
    private int importantIntOffset;
    ...
    {
        this.vertexCount = 0;
        this.matchingImportantField = null;
        this.importantIntOffset = 0;
    }
    ...
    public void begin(int drawMode, VertexFormat format)
    {
        ...
        else
        {
            ...
            this.matchingImportantField = format.getElement(this.importantIntOffset);
    ...
    {
        int i = this.vertexCount * this.vertexFormat.getNextOffset() + this.vertexFormat.func_181720_d(this.importantIntOffset);

        switch (this.matchingImportantField.getType())
        {
            case FLOAT:
    ...
    private void putColorRGBA(int index, int red, int g, int b, int a)
    {
        if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN)
        {
            this.rawIntBuffer.put(index, a << 24 | b << 16 | g << 8 | red);
        }
        else
        {
            this.rawIntBuffer.put(index, red << 24 | g << 16 | b << 8 | a);
        }
    }
    ...
    public void addVertexData(int[] vertexData)
    {
        ...
        this.rawIntBuffer.position(this.size());
    ...
    private void posFinish()
    {
        ++this.importantIntOffset;
        this.importantIntOffset %= this.vertexFormat.getElementCount();
        this.matchingImportantField = this.vertexFormat.getElement(this.importantIntOffset);

        if (this.matchingImportantField.getUsage() == VertexFormatElement.EnumUsage.PADDING)
        {
            this.posFinish();
        }
    }
    ...
    public void finishDrawing()
    {
        ...
        else
        {
            ...
            this.byteBuffer.position(0);
            this.byteBuffer.limit(this.size() * 4);
        }
    }
